import Link from "next/link";
import { notFound } from "next/navigation";
import { prisma } from "@/lib/prisma";
import { deactivateVehicleAction } from "@/modules/vehicles/actions";

export default async function VehicleDetailPage({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const vehicle = await prisma.vehicle.findUnique({ where: { id } });

  if (!vehicle) {
    notFound();
  }

  return (
    <section className="mx-auto max-w-5xl">
      <Link className="text-sm font-semibold text-[#1f5f57]" href="/vehiculos">
        Volver a vehiculos
      </Link>
      <div className="mt-4 rounded-md border border-[#d6d3c8] bg-white p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-[#2f6f65]">
              Vehiculo
            </p>
            <h1 className="mt-2 text-3xl font-semibold">
              {vehicle.brand} {vehicle.model}
            </h1>
            <p className="mt-1 text-sm text-[#66727c]">{vehicle.status}</p>
          </div>
          <div className="flex gap-2">
            <Link
              className="rounded-md border border-[#c9c7bd] px-4 py-2 text-sm font-semibold"
              href={`/vehiculos/${vehicle.id}/editar`}
            >
              Editar
            </Link>
            {vehicle.status !== "INACTIVE" ? (
              <form action={deactivateVehicleAction}>
                <input name="id" type="hidden" value={vehicle.id} />
                <button
                  className="rounded-md border border-[#b65f5f] px-4 py-2 text-sm font-semibold text-[#8a1f1f]"
                  type="submit"
                >
                  Desactivar
                </button>
              </form>
            ) : null}
          </div>
        </div>

        <dl className="mt-8 grid gap-4 sm:grid-cols-2">
          <Detail label="VIN" value={vehicle.vin} />
          <Detail label="Marca" value={vehicle.brand} />
          <Detail label="Modelo" value={vehicle.model} />
          <Detail label="Anio" value={vehicle.year.toString()} />
          <Detail label="Condicion" value={vehicle.condition} />
          <Detail
            label="Precio"
            value={`${vehicle.currency} ${vehicle.price.toString()}`}
          />
          <Detail label="Color" value={vehicle.color ?? "-"} />
          <Detail label="Tipo" value={vehicle.type ?? "-"} />
        </dl>
      </div>
    </section>
  );
}

function Detail({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-md border border-[#ebe8df] p-4">
      <dt className="text-xs font-semibold uppercase tracking-wide text-[#66727c]">
        {label}
      </dt>
      <dd className="mt-2 text-sm font-medium">{value}</dd>
    </div>
  );
}
